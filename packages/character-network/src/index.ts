...
class Network {
  ...
  pingPongRate: number = 1500;
  heartBeatRate: number = 3500;
  packetsUpdateRate: number = (1 / 30) * 1000;

  constructor() {
    setInterval(this.sendPlayerUpdates.bind(this), this.packetsUpdateRate);
    setInterval(this.pingClients.bind(this), this.pingPongRate);
    setInterval(this.heartBeat.bind(this), this.heartBeatRate);
  }

  heartBeat() {
    const now = Date.now();
    this.clientLastPong.forEach((clientLastPong, id) => {
      if (now - clientLastPong > this.heartBeatRate) {
  ...
  connectClient(socket: WebSocket) {
    ...
    this.clients.set(id, {
      socket: socket,
      update: { id, location: new Vector3(), rotation: new Vector2(), state: "idle" },
    });
